...
public class PointValidator {
     ...
     * Any incrementing means that the line of point is not a valid point.
     * @param arrayOfPoints to check if it is has valid points coordinate
    ...
    public boolean isValidPoint(String[] arrayOfPoints) {
        ...
        for (String arrayOfPoint : arrayOfPoints) {
            ...
            if(delimiter != FrequentlyUsedRegex.INDEX_ZERO){
                if (!(numericValidator.isNumeric(arrayOfPoint.substring(FrequentlyUsedRegex.INDEX_ZERO, delimiter)))) {
                    invalidCoordinate++;
                }else if (!numericValidator.isNumeric(arrayOfPoint.substring(delimiter + FrequentlyUsedRegex.NEXT_INDEX))){
                    invalidCoordinate++;
                }
            }
        }
